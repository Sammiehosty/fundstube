          ...
          <div className="lg:col-span-3">
            <div className="bg-white/5 backdrop-blur-xl border border-white/10 rounded-[2rem] p-8 shadow-2xl overflow-hidden relative group">
              ...
              <div className="flex items-center gap-3 mb-8">
                ...
                <div>
                  <h2 className="text-xl font-black tracking-tight uppercase">Network Earning History</h2>
              ...
              <div className="overflow-x-auto">
                <table className="w-full border-collapse">
                  ...
                  <tbody className="divide-y divide-white/5">
                    {accessCodes.filter(c => c.profile && c.profile.transactions.some(t => t.type === 'earning' || t.type === 'bonus')).length === 0 ? (
                      <tr>
                      ...
                      accessCodes.flatMap(c => 
                        (c.profile?.transactions || [])
                          .filter(t => t.type === 'earning' || t.type === 'bonus')
                          .map(t => ({ ...t, user: c.user, code: c.code }))
                      )
                      ...
                      .map((reward, i) => (
                        <tr key={i} className="group hover:bg-white/[0.02] transition-all">
                          <td className="py-4 px-2">
                             <div className="font-black text-xs text-white uppercase">{reward.user}</div>
                             <div className="text-[9px] text-slate-600 font-bold font-mono">NODE_{reward.code}</div>
                          </td>
                          <td className="py-4 px-2">
                          ...
                          <td className="py-4 px-2">
                             ...
                             <div className="text-[8px] text-slate-700 font-black uppercase">{new Date(reward.timestamp).toLocaleTimeString()}</div>
                          </td>
                          <td className="py-4 px-2 text-right">
                             <div className="font-black text-emerald-400 text-sm tracking-tight">+₦{reward.amount.toLocaleString()}</div>
                          </td>
                        </tr>
                      ))
                    )}
